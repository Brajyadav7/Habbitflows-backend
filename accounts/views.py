...
class CustomSchemeRedirect(HttpResponseRedirect):
    allowed_schemes = ['http', 'https', 'habitflows']
...
def google_login(request):
    ...
    request.session['google_platform'] = platform
    params = {
...
def google_callback(request):
    ...
    error = request.GET.get('error')
    platform = request.session.get('google_platform', 'web')
    if error or not code:
        err_url = 'habitflows://oauth?error=google_cancelled' if platform == 'mobile' else 'http://localhost:54321/?error=google_cancelled'
        return CustomSchemeRedirect(err_url) if platform == 'mobile' else redirect(err_url)
    ...
    try:
        ...
        err_url = 'habitflows://oauth?error=google_failed' if platform == 'mobile' else 'http://localhost:54321/?error=google_failed'
        return CustomSchemeRedirect(err_url) if platform == 'mobile' else redirect(err_url)
    ...
    if platform == 'mobile':
        return CustomSchemeRedirect(f'habitflows://oauth?sessionid={session_key}')
    return redirect(f'http://localhost:54321/?sessionid={session_key}')
