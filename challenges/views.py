...
def delete_challenge(request, challenge_id):
    ...
    try:
        ...
        return JsonResponse({'error': 'Not found'}, status=404)
    if ch.creator != request.user:
        return JsonResponse({'error': 'Only the creator can delete this challenge'}, status=403)
    ch.delete()
    return JsonResponse({'ok': True})
...
def search_users_for_invite(request):
    """Search all users by name or username for challenge invite."""
    from social.models import UserProfile
    ...
    profiles = UserProfile.objects.filter(
        ...
        Q(username__icontains=q) | Q(user__full_name__icontains=q)
